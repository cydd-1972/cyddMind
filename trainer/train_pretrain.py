...
from model.model import CyddMindConfig
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):   # 一个epoch的总迭代步数（batch数量）
    # 前向-->loss-->反向-->更新
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        lr = get_lr(
            ...
            args.learning_rate  # 最大学习率
        )

        # 把优化器里所有参数组的学习率都设置为新的lr
        ...
        with autocast_ctx:
            ...
            # 用掩码选择性地计算有效token的损失，然后取平均
            loss = (loss * loss_mask).sum() / loss_mask.sum()

            # 为什么需要aux_loss？辅助损失
            # 在某些复杂模型（如MoE、Sparse模型）中，除了主任务损失，
            # 还有额外的正则化损失或平衡损失
            loss += res.aux_loss

            # 按梯度累积步数缩放损失
                # 问题：GPU内存有限，无法放入大的batch_size
                # 解决方案：用小batch_size多次累积梯度，模拟大batch_size效果

                # 示例：
                # 目标：有效的batch_size = 32
                # 实际：单GPU内存只能放 batch_size = 8

                # 梯度累积方案：
                # 1. 用 batch_size=8 训练4步
                # 2. 每步累积梯度，不更新参数
                # 3. 第4步后，用累积的梯度更新参数
                # 效果相当于用 batch_size=32 训练
            loss = loss / args.accumulation_steps

        # 为什么要缩放？
        # FP16的表示范围有限（约 6.1e-5 到 65504）
        # 深度学习梯度通常很小（如 1e-7），在FP16中会变成0（下溢）
        # 放大后：1e-7 × 4096 = 4.096e-4，FP16可以正常表示
